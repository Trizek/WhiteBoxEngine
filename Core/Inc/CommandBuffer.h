...
{
public:
	CThreadedCommandBuffer( size_t capacity = 1024u )
		: m_capacity(capacity)
		, m_mask(capacity-1)
		, m_bottom(0)
		, m_top(0)
	{
		m_commands = (CCommand*)new char[ sizeof(CCommand) * capacity ]; // Don't construct the commands
	}

	~CThreadedCommandBuffer()
	{
		Clear();
		delete m_commands; // Don't destroy the commands !
	}
	...
	{
		CCommand::AddCommandToBuffer< TCommandObj >( m_commands + (m_bottom & m_mask), commandObj );
		++m_bottom;
	}
	...
	void	ExecuteCommands()
	{
		...
		while ( cursor < bottom )
		{
			CCommand* pCommand = m_commands + (cursor & m_mask);
			pCommand->ExecuteCommand();
			pCommand->~CCommand();
			++cursor;
		}
