...
CWorkerThread gpuThread, cpuThread;
CThreadDoubleBuffer	doubleBuffer;
...
public:
	...
	void Run()
	{
		...
		for (;;)
		{
			CCommandBuffer& commands = doubleBuffer.GetReceivedCommands();
...
public:
	...
	void Run()
	{
		...
		for (;;)
		{
			...
			CCommandBuffer& commands = doubleBuffer.LockSendCommandBuffer();
			...
			{
				// add commands
				new (commands.AddCommandToBuffer<C1>()) C1(frame);
				new (commands.AddCommandToBuffer<C2>()) C2("toto");
			...
			doubleBuffer.UnlockSendCommandBuffer();
		}
	}
};
...
cpuThread.Run( CMemberCallback().Bind<Cpu, &Cpu::Run>(cpu) );
gpuThread.Run(CMemberCallback().Bind<Gpu, &Gpu::Run>(gpu));

cpuThread.WaitForFinish();
...
void reshape(int width, int height) {
	...
	WhiteBox::gVars->pRenderer->ComputeProjectionMatrix( 0.1f, 100.0f, h, w, projMatrix );
	WhiteBox::gVars->pRenderer->SetProjectionMatrix( projMatrix );
...
//	pSubMesh->SetIndices( 3, indices );
	...
	//pMesh = new WhiteBox::CMesh();
		...
		/*
	WhiteBox::TFileHandle file = WhiteBox::gVars->pFileSystem->OpenFile( "Ezio.msh", true, false );
	
	size_t fileSize = WhiteBox::gVars->pFileSystem->GetFileSize( file );
	
	char* fileData = new char[ fileSize ];
	WhiteBox::gVars->pFileSystem->Read( file, 1, fileSize, fileData );
	...
	WhiteBox::gVars->pFileSystem->CloseFile( file );
	
	file = WhiteBox::gVars->pFileSystem->OpenFile( "vanquish.msh", true, false );
	
	fileSize = WhiteBox::gVars->pFileSystem->GetFileSize( file );
